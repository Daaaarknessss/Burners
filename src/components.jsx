...
export function BurnerArt({ ignited = false, size = 220 }) {
  return (
    <svg viewBox="0 0 240 240" width={size} height={size} className={ignited ? 'flicker' : ''} style={{ display: 'block', color: 'var(--ink)' }}>
      <defs>
      ...
      <circle cx="120" cy="120" r="108" fill="none" stroke="currentColor" strokeWidth="5" />
      <circle cx="120" cy="120" r="100" fill="none" stroke="currentColor" strokeWidth="1.5" strokeDasharray="2 6" />
      ...
      <circle cx="120" cy="120" r="58" style={{ fill: 'var(--paper)' }} stroke="currentColor" strokeWidth="4" />
      <circle cx="120" cy="120" r="58" fill="url(#halftone-coil)" opacity={ignited ? 0.0 : 0.55} />
      <circle cx="120" cy="120" r="44" fill="none" stroke="currentColor" strokeWidth="3" />
      <circle cx="120" cy="120" r="30" fill="none" stroke="currentColor" strokeWidth="3" />
          ...
          {[0, 60, 120, 180, 240, 300].map((deg, i) => (
            <g key={i} transform={`rotate(${deg} 120 120)`}>
              <path
                ...
                style={{ fill: 'var(--red)' }} stroke="currentColor" strokeWidth="3.5" strokeLinejoin="round"
              />
              <path
                d="M120,70 C113,84 109,88 110,98 C111,106 117,110 120,116 C123,110 129,106 130,98 C131,88 127,84 120,70 Z"
                style={{ fill: 'var(--paper)' }} stroke="currentColor" strokeWidth="2" strokeLinejoin="round"
              />
            </g>
          ))}
          {[30, 90, 150, 210, 270, 330].map((deg, i) => (
            <g key={i} transform={`rotate(${deg} 120 120)`}>
              <path d="M120,80 C114,92 112,96 114,104 C116,110 120,112 120,116 C120,112 124,110 126,104 C128,96 126,92 120,80 Z" fill="currentColor" />
            </g>
          ))}
      ...
      {ignited && (
        <g>
          <circle cx="120" cy="120" r="6" style={{ fill: 'var(--paper)' }} />
          <circle cx="120" cy="120" r="3" style={{ fill: 'var(--red)' }} />
        </g>
      )}
    </svg>
  )
...
export function MiniFlame({ size = 20, lit = true }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} style={{ display: 'block', color: 'var(--ink)' }}>
      <path
        d="M12 2 C10 6 7 8 7 13 a5 5 0 0 0 10 0 C17 9 14 7 12 2 Z"
        style={{ fill: lit ? 'var(--red)' : 'var(--paper)' }}
        stroke="currentColor" strokeWidth="2" strokeLinejoin="round"
      />
      <path d="M12 9 C11 11 10 12 10 14 a2 2 0 0 0 4 0 C14 12 13 11 12 9 Z" fill="currentColor" />
    </svg>
  )
}
